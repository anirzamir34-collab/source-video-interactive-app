...
- Çerez gerektirmeyen HTTPS video kaynaklarında önce CORS kurallarına uygun, kimlik bilgisi göndermeyen doğrudan indirme denenir. Başlıklar 4 saniyede gelmezse, CORS/HTTP hatası, yanlış içerik/boyut veya kesilen akış varsa geçici dosya temizlenerek mevcut Render proxy aktarımına dönülür. Depolama yetersizliği, boyut sınırı ve kullanıcının iptali ikinci indirme başlatmaz. HLS/DASH ve çerez isteyen kaynaklar mevcut proxy akışını korur. Kaynak video yeniden kodlanmaz veya düşük çözünürlükle değiştirilmez. Ağ ve kaynak site kısıtları nedeniyle sabit indirme hızı garantisi yoktur. İlgili Fetch davranışı: https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch
