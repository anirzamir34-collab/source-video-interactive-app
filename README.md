...
- URL arayüzünde “Bul ve indir” önce medya adresini sunucuda bulur, ardından dosyanın tamamını CORS kurallarına uygun ve kimlik bilgisi göndermeyen HTTPS isteğiyle doğrudan tarayıcıya indirir. Render proxy üzerinden video önizlemesi, tarayıcı akış kontrolü veya indirmeye otomatik dönüş yapılmaz. Yerel dosya tamamlanınca oynatıcı açılır ve analiz kullanılabilir olur; ses ve kareler bu dosyadan hazırlanır. Sonraki analiz/dublaj servis istekleri devam eder.
- Doğrudan bağlantı başlıkları için 12 saniye beklenir. Kaynak CORS/HTTP erişimini engellerse, bağlantı kesilirse veya kaynak HLS/DASH gibi birleştirme gerektiren akışsa tarayıcıda video/kaynak sayfa açma ve indirilen dosyayı seçme düğmeleri gösterilir. Uygulama tarayıcının İndirilenler klasöründeki dosyayı kendiliğinden okuyamaz; kullanıcı dosyayı seçmelidir. Depolama/boyut sınırları korunur; kalite düşürülmez ve yeniden kodlama yapılmaz. Hız kaynak site ve bağlantıya bağlıdır. İlgili Fetch davranışı: https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API/Using_Fetch
