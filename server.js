...
- Use one canonical positionId for the entire video for each genuinely identical body configuration; wording, tempo, camera angle or minor pose variation must never create a second positionId.
- Before assigning an internal movement to a positionId, verify that the parent position is visibly present at the movement midpoint. If not, reject that movement instead of attaching it to another position.
- Oral activity, manual activity, undressing and transitions must never appear beneath a penetrative-position parent; penetrative movements must never appear beneath an oral/manual parent.
- Each internal movement time range must be fully contained within exactly one parent position time range and may not overlap a different position.
- Return positions in chronological first-appearance order and movements in chronological order inside only their verified parent.
...
- Perform two complete audio passes: first for chronology and words, second specifically for missed short lines, quiet voices, female voices, overlaps and dialogue around scene transitions.
- Never merge different speakers into one segment. Split immediately whenever the speaker changes.
- Preserve speaker identity across consecutive shots by voice evidence; a camera cut must not reset or swap the speaker.
- Include short intelligible replies and interjections. If gender is uncertain, keep the line with gender uncertain instead of omitting it.
- Separate overlapping speakers into individual chronological segments when both are intelligible.
- Keep each dialogue segment aligned to one utterance and preferably no longer than 8 seconds; split longer speech at natural pauses for accurate dubbing synchronization.
- Recheck the final timeline for uncovered intelligible speech gaps before returning JSON.
