...
export function videoResolutionFailure(detail) {
  ...
  if (/VK_PLAYER_UNAVAILABLE/.test(value)) return { reason: 'VIDEO_SOURCE_UNAVAILABLE', message: 'VK bu video için oynatıcının kullanılamadığını bildirdi. Video bu bağlantı üzerinden sunucuya aktarılamıyor. Elindeki video dosyasını cihazından yükleyebilirsin.' };
  ...
  if (/VK_PLAYER_METADATA_MISSING/.test(value)) return { reason: 'VIDEO_EXTRACTOR_FAILED', message: 'VK sayfası yanıt verdi ancak oynatıcı bilgisi okunamadı. Bağlantının hatalı olduğu anlamına gelmez; kaynak oynatıcı henüz çözümlenemedi.' };
  if (/VIDEO_EXTRACTOR_(?:PROCESS_FAILED|OUTPUT_LIMIT|INVALID_RESPONSE)/.test(value)) return { reason: 'VIDEO_EXTRACTOR_FAILED', message: 'Video çıkarıcısı kaynak sitenin yanıtını işlerken hata verdi. Bu, bağlantının yanlış olduğu anlamına gelmez.' };
...
export async function probeVideoSource(candidate, referer, { fetchPublicUrl, signal, cookie = '', userAgent = '' } = {}) {
  ...
  try {
    ...
    let fetched = await fetchPublicUrl(candidate, { headers, timeoutMs: 25000, signal });
    response = fetched.response;
    // Some otherwise playable servers reject a Range probe.
    if ([405, 416].includes(response.status)) {
      await response.body?.cancel();
      delete headers.Range;
      fetched = await fetchPublicUrl(candidate, { headers, timeoutMs: 25000, signal });
      response = fetched.response;
    }
...
export async function resolveVideoPage(startUrl, { fetchPublicUrl, signal = AbortSignal.timeout(60000), onPage = () => {} } = {}) {
  ...
  while (queue.length && visited.size < 12) {
    ...
    try {
      ...
      const fetched = await fetchPublicUrl(page.url, { headers, timeoutMs: 25000, signal });
      response = fetched.response;
...
export async function resolveVideoUrl(startUrl, {
  fetchPublicUrl, extractPage, signal = AbortSignal.timeout(300000)
} = {}) {
  const pages = new Map([[startUrl, { url: startUrl, referer: startUrl, depth: 0 }]]);
  const errors = [];
  try {
    const direct = await resolveVideoPage(startUrl, {
      fetchPublicUrl, signal: AbortSignal.any([signal, AbortSignal.timeout(60000)]),
  ...
  if (extractPage) {
    ...
    for (const page of attempts) {
      ...
      signal.throwIfAborted();
      try {
        const timeoutMs = vk ? 90000 : 60000;
