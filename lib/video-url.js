...
export async function resolveVideoUrl(startUrl, {
  ...
  try {
    const direct = await resolveVideoPage(startUrl, {
      fetchPublicUrl, signal: AbortSignal.any([signal, AbortSignal.timeout(20000)]),
  ...
  if (extractPage) {
    ...
    for (const page of [...nested, pages.get(startUrl)]) {
      signal.throwIfAborted();
      try {
        const attemptSignal = AbortSignal.any([signal, AbortSignal.timeout(20000)]);
        const candidate = await extractPage(page.url, { referer: page.referer, signal: attemptSignal });
      ...
      } catch (error) { errors.push(String(error?.stderr || error?.message || error).slice(0, 900)); }
    }
  }
  signal.throwIfAborted();
